 ...
 *  @version 1.2.0  2017-02-02  Alvin Lai     Completed containsVowel and reverse methods
 *  @version 1.3.0  2017-02-07  Alvin Lai     Completed evensOnly, oddsOnly, and the noDupes methods
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
...
public class StringStuff {
   ...
   */
   public static boolean containsVowel( String word ) {
      //converts every character in string to lower case
   ...
   */
   public static String evensOnly( String input ) {
      /**
       * Parse every character in the input string and compare them to every character in the
       * that contains all the even indexed alphabets/symbols. If the character in the input is
       * in the "evens" string, append the null StringBuffer with that character, then output the
       * final result
       */
      String evens = "bdfhjlnprtvxzBDFHJLNPRTVXZ13^";
      StringBuffer output = new StringBuffer( "" );
      for ( int i = 0; i < input.length(); i++ ) {
         for ( int j = 0; j < evens.length(); j++ ) {
            if ( input.charAt(i) == evens.charAt(j) ) {
               output.append(evens.charAt(j));
            }
         }
      }
   ...
   */
   public static String oddsOnly( String input ) {
      String odds = "acegikmoqsuwyACEGIKMOQSUWY";
      StringBuffer output = new StringBuffer( "" );
      for ( int i = 0; i < input.length(); i++ ) {
         for ( int j = 0; j < odds.length(); j++ ) {
            if ( input.charAt(i) == odds.charAt(j) ) {
               output.append(odds.charAt(j));
            }
         }
      }
      System.out.println( output );
   ...
   */
   public static String evensOnlyNoDupes( String input ) {
      /**
       * Same idea as evensOnly method, except we now remove the character at the specific index
       * in the StringBuffer containing the characters we want if the characters are the same. 
       * This updates the string by removing characters we've already used to ensure that we don't
       * duplicate any characters.
       */
      StringBuffer evens = new StringBuffer ( "bdfhjlnprtvxzBDFHJLNPRTVXZ13^" );
      StringBuffer output = new StringBuffer( "" );
      for ( int i = 0; i < input.length(); i++ ) {
         for ( int j = 0; j < evens.length(); j++ ) {
            if ( input.charAt(i) == evens.charAt(j) ) {
               output.append(evens.charAt(j));
               evens.deleteCharAt(j);
            }
         }
      }
      System.out.println( output );
   ...
   */
   public static String oddsOnlyNoDupes( String input ) {
      StringBuffer odds = new StringBuffer ( "acegikmoqsuwyACEGIKMOQSUWY" );
      StringBuffer output = new StringBuffer( "" );
      for ( int i = 0; i < input.length(); i++ ) {
         for ( int j = 0; j < odds.length(); j++ ) {
            if ( input.charAt(i) == odds.charAt(j) ) {
